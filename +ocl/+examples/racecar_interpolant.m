...
%
function [solution,times,ocp] = racecar_interpolant
  ...
  ocp = ocl.Problem([], @varsfun, @daefun, ...
    'gridcosts', @gridcosts, ...
    'gridconstraints', @(ch,k,K,x,p)gridconstraints(ch,k,K,x,p,center_line_lut), ...
    'N', 50, 'nlp_casadi_mx', true);
...
function gridconstraints(ch,~,~,x,p,center_line_lut)
  % speed constraint
  ...
  % road bounds
  y_center = center_line_lut(x.x.value);
  y_max =  0.5*p.road_bound + y_center;
  y_min =  -0.5*p.road_bound + y_center;
  ch.add(x.y,'<=',y_max);
  ch.add(x.y,'>=',y_min);
end
